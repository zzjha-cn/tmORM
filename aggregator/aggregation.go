...
func (a *Aggregator[T]) Match(filter bson.M) *Aggregator[T] {
	return a.AddStage(bson.M{"$match": filter})
}
...
func (a *Aggregator[T]) Group(id interface{}, fields bson.M) *Aggregator[T] {
	...
	return a.AddStage(bson.M{"$group": groupStage})
}

// Sort 排序阶段
func (a *Aggregator[T]) Sort(sorts bson.D) *Aggregator[T] {
	return a.AddStage(bson.M{"$sort": sorts})
}
...
func (a *Aggregator[T]) Project(projection bson.M) *Aggregator[T] {
	return a.AddStage(bson.M{"$project": projection})
}
...
func (a *Aggregator[T]) Limit(limit int64) *Aggregator[T] {
	return a.AddStage(bson.M{"$limit": limit})
}

// Skip 跳过指定数量
func (a *Aggregator[T]) Skip(skip int64) *Aggregator[T] {
	return a.AddStage(bson.M{"$skip": skip})
}
...
func (a *Aggregator[T]) Unwind(path string, preserveNullAndEmptyArrays ...bool) *Aggregator[T] {
	...
	return a.AddStage(bson.M{"$unwind": unwindStage})
}
...
func (a *Aggregator[T]) AddFields(fields bson.M) *Aggregator[T] {
	return a.AddStage(bson.M{"$addFields": fields})
}
...
func (a *Aggregator[T]) Facet(facets bson.M) *Aggregator[T] {
	return a.AddStage(bson.M{"$facet": facets})
}
...
func (a *Aggregator[T]) Bucket(groupBy interface{}, boundaries []interface{}, defaultBucket interface{}, output bson.M) *Aggregator[T] {
	...
	return a.AddStage(bson.M{"$bucket": bucketStage})
}
...
func (a *Aggregator[T]) Count(field string) *Aggregator[T] {
	return a.AddStage(bson.M{"$count": field})
}
...
func (gb *GroupBuilder) Sum(field, sourceField string) *GroupBuilder {
	gb.fields[field] = bson.M{"$sum": "$" + sourceField}
	return gb
}
...
func (a *Aggregator[T]) GroupByYear(dateField string) *GroupBuilder {
	return a.NewGroupBuilder(bson.M{"year": bson.M{"$year": "$" + dateField}})
}
...
func (e *AggExpr) Size() *AggExpr {
	return &AggExpr{expr: bson.M{"$size": e.expr}}
}

// Type 类型检查
func (e *AggExpr) Type() *AggExpr {
	return &AggExpr{expr: bson.M{"$type": e.expr}}
}

// ToString 转换为字符串
func (e *AggExpr) ToString() *AggExpr {
	return &AggExpr{expr: bson.M{"$toString": e.expr}}
}

// ToInt 转换为整数
func (e *AggExpr) ToInt() *AggExpr {
	return &AggExpr{expr: bson.M{"$toInt": e.expr}}
}

// ToDouble 转换为浮点数
func (e *AggExpr) ToDouble() *AggExpr {
	return &AggExpr{expr: bson.M{"$toDouble": e.expr}}
}

// ToDate 转换为日期
func (e *AggExpr) ToDate() *AggExpr {
	return &AggExpr{expr: bson.M{"$toDate": e.expr}}
}
...
func (e *AggExpr) Year() *AggExpr {
	return &AggExpr{expr: bson.M{"$year": e.expr}}
}

// Month 月份
func (e *AggExpr) Month() *AggExpr {
	return &AggExpr{expr: bson.M{"$month": e.expr}}
}

// Day 日期
func (e *AggExpr) Day() *AggExpr {
	return &AggExpr{expr: bson.M{"$dayOfMonth": e.expr}}
}

// Hour 小时
func (e *AggExpr) Hour() *AggExpr {
	return &AggExpr{expr: bson.M{"$hour": e.expr}}
}

// Minute 分钟
func (e *AggExpr) Minute() *AggExpr {
	return &AggExpr{expr: bson.M{"$minute": e.expr}}
}

// Second 秒
func (e *AggExpr) Second() *AggExpr {
	return &AggExpr{expr: bson.M{"$second": e.expr}}
}
...
func (e *AggExpr) DateToString(format string, timezone ...string) *AggExpr {
	...
	return &AggExpr{expr: bson.M{"$dateToString": dateToString}}
}
...
func AggConcat(exprs ...*AggExpr) *AggExpr {
	...
	return &AggExpr{expr: bson.M{"$concat": args}}
}
...
func (e *AggExpr) ToLower() *AggExpr {
	return &AggExpr{expr: bson.M{"$toLower": e.expr}}
}

// ToUpper 转大写
func (e *AggExpr) ToUpper() *AggExpr {
	return &AggExpr{expr: bson.M{"$toUpper": e.expr}}
}

// StrLen 字符串长度
func (e *AggExpr) StrLen() *AggExpr {
	return &AggExpr{expr: bson.M{"$strLenCP": e.expr}}
}
